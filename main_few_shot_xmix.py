...
class Processor:
    ...
    def load_data(self):
        ...
        Eval_Feeder = import_class(self.arg.eval_feeder)
        self.data_loader = dict()
        if self.arg.phase == 'train':
            aux_dataset = Feeder(**self.arg.aux_feeder_args, class_group=self.arg.one_shot_class_group)

            self.data_loader['aux'] = torch.utils.data.DataLoader(
                dataset=aux_dataset,
            ...
            self.train_modality = dict(
                bone=self.data_loader['aux'].dataset.bone,
                vel=self.data_loader['aux'].dataset.vel,
            )
        # Note that class_group is not the same as one_shot_class_group.
        self.data_loader['eval'] = torch.utils.data.DataLoader(
            dataset=Eval_Feeder(**self.arg.eval_feeder_args),
        ...
        self.test_modality = dict(
            bone=self.data_loader['eval'].dataset.bone,
            vel=self.data_loader['eval'].dataset.vel,
        )

        self.data_loader['anchor'] = torch.utils.data.DataLoader(
            dataset=Eval_Feeder(**self.arg.anchor_feeder_args),
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            if self.arg.x_SADP:
                ...
                pair_data, pair_label, pair_idx = self.data_loader['aux'].dataset.get_pair_data(index, mode='SADP')
                with torch.no_grad():
            ...
            if self.arg.x_DASP:
                ...
                timer['model'] += self.split_time()
                pair_data, pair_label, pair_idx = self.data_loader['aux'].dataset.get_pair_data(index, mode='DASP')
                with torch.no_grad():
            ...
            tot_loss, tot_acc = self.model(data, x_dasp=pair_data_DASP,x_sadp=pair_data_SADP, label=label,
    ...
    def eval(self, epoch, save_score=False, wrong_file=None, result_file=None, mask_subject=False):
        ...
        for (data, label, index) in self.data_loader['anchor']:
            with torch.no_grad():
                ...
                if mask_subject:
                    # mask-out subject feature which is the first half of the feature
                    anchor_embedding = anchor_embedding[:, anchor_embedding.shape[1] // 2:]
            break
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            with torch.no_grad():
                ...
                if mask_subject:
                    # mask-out subject feature which is the first half of the feature
                    embedding = embedding[:, embedding.shape[1] // 2:]
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if (epoch + 1) % self.arg.eval_interval == 0 or (epoch + 1) == self.arg.num_epoch:
                    ...
                    if self.arg.eval_mask_subject:
                        self.print_log("Eval epoch: {} with subject feature masked".format(epoch + 1))
                        self.eval(epoch, save_score=self.arg.save_score, mask_subject=True)


            # test the best model
            print("Best accuracy: ", self.best_acc, ', epoch:', self.best_acc_epoch)
            ...
            state_dict = self.model.state_dict()
            weights = OrderedDict([[k.split('module.')[-1], v.cpu()] for k, v in state_dict.items()])
            torch.save(weights, os.path.join(self.arg.work_dir, 'best_acc.pt'))
            ...
            self.eval(epoch=0, save_score=self.arg.save_score, wrong_file=wf, result_file=rf)
            wrong_analyze(wf, rf)
            self.print_log('Done.\n')
