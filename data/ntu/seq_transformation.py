...
def get_indices(performer, camera, evaluation='CS'):
    ...
    if evaluation == 'CS':  # Cross Subject (Subject IDs)
        ...
        test_df:pd.DataFrame = df.iloc[test_indices]

        # Get indices of training data
        for train_id in train_ids:
            temp = np.where(performer == train_id)[0]  # 0-based index
            train_indices = np.hstack((train_indices, temp)).astype(np.int)
        train_df:pd.DataFrame = df.iloc[train_indices]
    ...
    # rearrange column "index" from 0 to len(train_indices) or len(test_indices) for test_df and train_df
    train_df['index'] = np.arange(0, len(train_indices))
    test_df['index'] = np.arange(0, len(test_indices))

    print(test_df)
    
    # save data information as csv
    train_df.to_csv(f'train_indices_info_NTU60_{evaluation}.csv', index=False)
    test_df.to_csv(f'test_indices_info_NTU60_{evaluation}.csv', index=False)
