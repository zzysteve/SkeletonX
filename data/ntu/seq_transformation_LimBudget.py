...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path, num_samples_per_class=None):
    train_indices, test_indices = get_indices(performer, camera, evaluation, num_samples_per_class)
    # Select validation set from training set
    # train_indices, val_indices = split_train_val(train_indices, m)

    if num_samples_per_class is not None:
        save_name = f'NTU60_{evaluation}_{num_samples_per_class}.npz'
    else:
...
def get_indices(performer, camera, evaluation='CS', num_samples_per_class=None):
    ...
    if evaluation == 'CS':  # Cross Subject (Subject IDs)
        ...
        for i in range(60):
            ...
            if num_samples_per_class > len(df_temp):
                raise ValueError('num_samples_per_class should be less than the number of samples in each class')
        ...
        raise ValueError('Invalid evaluation type: %s' % evaluation)
    return train_indices, test_indices


if __name__ == '__main__':
    camera = np.loadtxt(camera_file, dtype=np.int)  # camera id: 1, 2, 3
    setup = np.loadtxt(setup_file, dtype=np.int)  # setup id: 1, 2, 3
    ...
    # split_dataset(None, label, performer, camera, 'LimBudget', save_path, num_samples_per_class=50)
    # exit()
    ...
    for num_samples_per_class in [10, 20, 30, 50]:
        split_dataset(skes_joints, label, performer, camera, 'LimBudget', save_path, num_samples_per_class=num_samples_per_class)
