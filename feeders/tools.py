...
def valid_crop_resize(data_numpy, valid_frame_num, p_interval, window):
    ...
    begin = 0
    # special treatment for valid_frame_num == 0
    if valid_frame_num < 1:
        valid_frame_num = window # span the whole space
    end = valid_frame_num
...
def random_noise(data_numpy, sigma=0.01, torch_array=False):
    # input: C,T,V,M
    noise = np.random.normal(0, sigma, data_numpy.shape)
    # transform noise to type of data_numpy
    if torch_array:
