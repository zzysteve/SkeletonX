# This code is borrowed from [ST-GCN](https://github.com/yysijie/st-gcn)
import math
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_frame=64, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, num_set=3, metric_func=None):
        ...
        elif metric_func == 'ArcFace':
            self.fc = ArcMarginProduct(256, num_class)
        elif metric_func == 'CosFace':
            self.fc = AddMarginProduct(256, num_class)
        elif metric_func == 'SphereFace':
            self.fc = SphereProduct(256, num_class)
    ...
    def forward_to_hidden_feat(self, x):
        # return the hidden feature of the last layer, with shape [N, M, C, T*V]
        N, C, T, V, M = x.size()
        x = x.permute(0, 4, 3, 1, 2).contiguous().view(N, M * V * C, T)
        x = self.data_bn(x)
        x = x.view(N, M, V, C, T).permute(0, 1, 3, 4, 2).contiguous().view(N * M, C, T, V)
